...
void signalEventLoss_glue()
{
    ...
    TFile *fInputFile = new TFile("/home/sawan/Downloads/SigLossLHC24l1.root", "read");
    ...
    // string path1 = "higher-mass-resonances_f21525_All_id49001/hMChists";
    // string path2 = "higher-mass-resonances_f21525_sel8_id49001/hMChists";
    string path1 = "higher-mass-resonances_f21525_Vz_id49001/hMChists";
    string path2 = "higher-mass-resonances_f21525_VzSel8_id49001/hMChists";
    ...
    THnSparse *h2GenAll = (THnSparse *)fInputFile->Get(Form("%s/Genf17102", path1.c_str()));
    THnSparse *h2GenVzSel = (THnSparse *)fInputFile->Get(Form("%s/Genf17102", path2.c_str()));

    if (h2GenAll == nullptr || h2GenVzSel == nullptr)
    {
        cout << "Error reading histograms" << endl;
        return;
    }

    TH1F *hGenAll = (TH1F *)h2GenAll->Projection(1);  // projecting pt axis
    TH1F *hGenVzSel = (TH1F *)h2GenVzSel->Projection(1); // projecting pt axis
    ...
    for (int ipt = 0; ipt < sizePtBins; ipt++)
    {
        ...
        int lowpTbin = hGenAll->GetXaxis()->FindBin(ptbins[ipt] + 0.01);
        int highpTbin = hGenAll->GetXaxis()->FindBin(ptbins[ipt + 1] - 0.01);
        double genAll = hGenAll->Integral(lowpTbin, highpTbin);
        double genVzSel = hGenVzSel->Integral(lowpTbin, highpTbin);
        double signalLoss = 0.0;
        if (genVzSel > 0)
            signalLoss = genAll / genVzSel;
        hSignalLossf0->SetBinContent(ipt + 1, signalLoss);
    }
