...
void GlobalChi2(Int_t &npar, Double_t *gin, Double_t &fval, Double_t *par, Int_t iflag)
{
    ...
    fval += Chi2Hist(gKstar, 0.89556, beta, T, n, normKstar, KstarMin, KstarMax);
}
...
void BlastWaveFitWithKstar()
{
    ...
    TFile *fOutput = new TFile("BlastWaveFits/BlastWaveFitResultsKstar.root", "recreate");
    ...
    for (int im = 0; im < 10; im++)
    {
        ...
        minuit->SetParameter(6, "NormKstar", 500, 1, 1, 1e5);
        ...
        TF1 *fKstar = new TF1(Form("fKstar_%d-%d", multClases[im], multClases[im + 1]), BGBlastWave_Func, KstarMin, KstarMax, 5);
        fKstar->SetParameters(0.89556, betaArr[im], TArr[im], nArr[im], minuit->GetParameter(6));

        fPi->SetLineColor(kBlue + 1);
        fK->SetLineColor(kRed + 1);
        fP->SetLineColor(kGreen + 2);
        fKstar->SetLineColor(kBrown);

        fPi->SetLineWidth(3);
        fK->SetLineWidth(3);
        fP->SetLineWidth(3);
        fKstar->SetLineWidth(3);
        ...
        pt->Draw();
        cPiKp->SaveAs(Form("BlastWaveFits/BlastWaveFitKstar_%d-%d.png", multClases[im], multClases[im + 1]));
    ...
    cAvgBeta->SaveAs("BlastWaveFits/BlastWaveFitKstar_AvgBeta.png");
    ...
    grT->Draw("AP");
    grT->Write("grT");
    cT->SaveAs("BlastWaveFits/BlastWaveFitKstar_T.png");
    ...
    grN->Draw("AP");
    grN->Write("grN");
    cN->SaveAs("BlastWaveFits/BlastWaveFitKstar_n.png");
    ...
    grChi2->Draw("AP");
    grChi2->Write("grChi2");
    cChi2->SaveAs("BlastWaveFits/BlastWaveFitKstar_Chi2.png");
}
