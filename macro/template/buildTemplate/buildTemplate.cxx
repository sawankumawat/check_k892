...
void buildTemplate()
{
    ...
    TString Cenoutputfolder = Form("template/%s", kResBkg.c_str());
    ...
    const string recpath = "kstarqa/hInvMass/h3KstarMassRecCalib";
    ...
    TFile *outPutSigMinusTrue = new TFile(Form("%s/SignalMinusTrue.root", Cenoutputfolder.Data()), "RECREATE");
    ...
    for (Int_t ip = pt_start; ip < pt_end; ip++) // start pt bin loop
    {
        ...
        int lowbinCentRec = hRec->GetAxis(1)->FindBin(lowcent + 1e-5);
        int highbinCentRec = hRec->GetAxis(1)->FindBin(highcent - 1e-5);
