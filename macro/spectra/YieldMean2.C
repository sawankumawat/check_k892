...
void YieldMean_IntegralMean(TH1 *hdata, TH1 *hlo, TH1 *hhi, Double_t &integral, Double_t &mean, Double_t &extra, TH1 *hout, Bool_t printinfo = kFALSE);
...
TH1 *YieldMean(TH1 *hstat, TH1 *hsys, TF1 *f = NULL, Double_t min = 0., Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "0q", TString logfilename = "log.root", Double_t minfit = 0.0, Double_t maxfit = 10.0)
{
  ...
  TH1 *hout = new TH1D("hout", "", 11, 0, 11);
  TH1 *hlo, *hhi;
  ...
  YieldMean_IntegralMean(htot, hlo, hhi, integral, mean, extra, hout, kTRUE);
  ...
  for (Int_t irnd = 0; irnd < 100; irnd++)
  {
    ...
    /* integrate */
    YieldMean_IntegralMean(hrnd, hrndlo, hrndhi, integral, mean, extra, hout);
  ...
  YieldMean_IntegralMean(hhigh, hlo, hhi, integral, mean, extra, hout);
  ...
  YieldMean_IntegralMean(hhard, hlo, hhi, integral, mean, extra, hout);
  ...
  YieldMean_IntegralMean(hlow, hlo, hhi, integral, mean, extra, hout);
  integral = TMath::Abs(integral - hout->GetBinContent(kYield));
  ...
  YieldMean_IntegralMean(hsoft, hlo, hhi, integral, mean, extra, hout);
  mean = TMath::Abs(mean - hout->GetBinContent(kMean));
...
void YieldMean_IntegralMean(TH1 *hdata, TH1 *hlo, TH1 *hhi, Double_t &integral, Double_t &mean, Double_t &extra, TH1 *hout, Bool_t printinfo)
{
  ...
  double ExtrapolationLowValue = (I - dataonly) / I;
  hout->SetBinContent(kExtrapolationLow, ExtrapolationLowValue);
  if (hhi)
  {
    for (Int_t ibin = 0; ibin < hhi->GetNbinsX(); ibin++)
    {
  ...
  double ExtrapolationHighValue = (I - dataonly) / I;
  hout->SetBinContent(kExtrapolationHigh, ExtrapolationHighValue);
}
