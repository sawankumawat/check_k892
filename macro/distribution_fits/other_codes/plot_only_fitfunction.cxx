...
Double_t Boltzmann2(double *x, double *par)
{
    double norm = par[0];
    double massks = 0.497;
    double n = par[1];
    double C = par[2];
    double y = norm * pow((x[0] + 0.002), n / 2) * pow(C, 3 / 2) * exp(-C * pow((x[0] + 0.002), n));
    return y;
}

int plot_only_fitfunction()
{
    ...
    //**********************Here we check the Boltzman fit function */
    TF1 *Boltzmann_fit = new TF1("Boltzmann_fit", Boltzmann2, 0, 10, 3);
    Boltzmann_fit->SetParameter(0, 100);
    Boltzmann_fit->SetParameter(1, 1);
    Boltzmann_fit->SetParameter(2, 10);
    Boltzmann_fit->Draw();
