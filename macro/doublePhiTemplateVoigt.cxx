...
void doublePhiTemplateVoigt()
{
    ...
    for (int ibin = 0; ibin < totalBins; ibin++)
    {
        ...
        // ================================================
        TF2 *f2D = new TF2(Form("f2D_bin%d", ibin), FitFunc2DVoigt, 1.0, 1.04, 1.0, 1.04, 7);
        ...
        if (!has_valid_seed)
        {
            ...
            f2D->SetParameter(3, 0.0012); // Gaussian resolution width
            f2D->SetParLimits(3, 0.0001, 0.0100);
            f2D->FixParameter(4, 0.00425); // Fixed Lorentzian natural width

            f2D->SetParameter(5, 1.0); // Pol2 p0
            f2D->SetParameter(6, 1.0); // Pol2 p1
            // f2D->SetParameter(7, 1.0); // Pol2 p2
        }
        else
        {
            ...
            f2D->SetParLimits(2, 1.016, 1.025);
            f2D->SetParLimits(3, 0.001, 0.0100);
        }
        ...
        cout << "Resolution       : " << f2D->GetParameter(3) << " +/- " << f2D->GetParError(3) << endl;
