...
void MakeInvQMPlot1_1to10_4BW_interference_final()
{
  MyStyle();
  gStyle->SetOptStat(0);
  gStyle->SetOptFit(0);
  ...
  */
  ...
  Double_t binw = hSigFit1->GetBinWidth(10);
  // calcultion for significance
  Double_t S1, Double_t B1;
  Double_t S2, Double_t B2;
  Double_t S3, Double_t B3;

  Double_t xmin_r1,xmax_r1;
  Double_t xmin_r2,xmax_r2;
  Double_t xmin_r3,xmax_r3;
  Double_t factor = 2.0;
  xmin_r1 = mass - (factor*width/1000.0);
  xmax_r1 = mass + (factor*width/1000.0);

  cout << "xmin1--" << xmin_r1 << "\t" << xmax_r1 << "---mass--" << mass << "\t" << width << endl;
  

  xmin_r2 = mass2 - (factor*width2/1000.0);
  xmax_r2 = mass2 + (factor*width2/1000.0);


  xmin_r3 = mass3 - (factor*width3/1000.0);
  xmax_r3 = mass3 + (factor*width3/1000.0);

  B1 = fBgOnly1->Integral(xmin_r1,xmax_r1)/binw;
  S1 = fSigOnly1->Integral(xmin_r1,xmax_r1)/binw;

  B2 = fBgOnly1->Integral(xmin_r2,xmax_r2)/binw;
  S2 = fSigOnly1->Integral(xmin_r2,xmax_r2)/binw;

  B3 = fBgOnly1->Integral(xmin_r3,xmax_r3)/binw;
  S3 = fSigOnly1->Integral(xmin_r3,xmax_r3)/binw;

  Double_t Significance1 = S1/(TMath::Sqrt(B1+S1));

  Double_t Significance2 = S2/(TMath::Sqrt(B2+S2));
    
  Double_t Significance3 = S3/(TMath::Sqrt(B3+S3));


  cout << "S1-----" << S1  << "\t" << "B1-----"  <<  B1  << "\t" << "Signi1-----" << Significance1 << endl;

  cout << "S2-----" << S2  << "\t" << "B2-----"  <<  B2  << "\t" << "Signi2-----" << Significance2 << endl;


   cout << "S3-----" << S3  << "\t" << "B3-----"  <<  B3  << "\t" << "Signi3-----" << Significance3 << endl;  
 ...
 // lp2->AddEntry((TObject*)0,"#bf{ALICE Preliminary}","");
  ...
  TF1 *fSigOnly2 = new TF1("fitFcn2",rBW,1.26,1.97,14);
