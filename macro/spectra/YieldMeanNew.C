...
namespace yieldmeannew
{
  ...
  void YieldMean_IntegralMean(TH1 *hdata, TH1 *hlo, TH1 *hhi, double &integral, double &mean, double &extra, Bool_t printinfo = kFALSE);
  ...
  int Fitter(TH1 *histo, TF1 *func, Option_t *opt, float range_min = 0, float range_max = 10);
  ...
  LogCustom logger(TLogLevel::DEBUG);
  TH1 *
  YieldMeanNew(TH1 *hstat, TH1 *hsys, TH1 *hsys_pt_uncorr, TH1 *hsys_pt_corr, TH1 *hsys_mult_corr, TF1 *&fout, TF1 *f = nullptr, double min = 0., double max = 10., double loprecision = 0.01, double hiprecision = 0.1, bool store_log = false, const char *logfilename = "log.root", const char *path = "", Option_t *opt = "0q", float range_min = 0, float range_max = 10)
  {
    ...
    /* create output histo */
    double integral, mean, extra, integral_ref, mean_ref;
    TH1 *hout = new TH1D(Form("hout_%s", hstat->GetName()), "", kExtra, 0, kExtra);
     ...
     */
    int fitres = Fitter(htot, f, opt, range_min, range_max);
    hout->SetBinContent(kExtra, extra);

    fout = new TF1(*f);
    fout->SetLineColor(kRed);
    logger.Get(TLogLevel::DEBUG) << "Fit sys+stat for " << f->GetName();
    logger.Get(TLogLevel::DEBUG) << "NDF=" << f->GetNDF() << " Chi^2=" << f->GetChisquare() << " Chi^2/NDF=" << f->GetChisquare() / f->GetNDF();

    hlo = YieldMean_LowExtrapolationHisto(htot, f, min, loprecision);
    hhi = YieldMean_HighExtrapolationHisto(htot, f, max, hiprecision);

    YieldMean_IntegralMean(htot, hlo, hhi, integral, mean, extra, true);
    ...
    /* fit with stat error */
    Fitter(hstat, f, opt, range_min, range_max);
    delete hlo;
    delete hhi;
    ...
    Fitter(hsys_pt_uncorr, f, opt, range_min, range_max);
    delete hlo;
    delete hhi;
    ...
    Fitter(hhigh_pt_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hhigh_pt_corr, hlo, hhi, integral, mean, extra);
    ...
    Fitter(hhard_pt_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hhard_pt_corr, hlo, hhi, integral, mean, extra);
    ...
    Fitter(hlow_pt_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hlow_pt_corr, hlo, hhi, integral, mean, extra);
    integral = TMath::Abs(integral - hout->GetBinContent(kYield));
    integral = sqrt(Sq(integral) + Sq(hout->GetBinContent(kYieldSysTot)));
    hout->SetBinContent(kYieldSysTot, integral);
    ...
    Fitter(hsoft_pt_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hsoft_pt_corr, hlo, hhi, integral, mean, extra);
    mean = TMath::Abs(mean - hout->GetBinContent(kMean));
    mean = sqrt(Sq(mean) + Sq(hout->GetBinContent(kMeanSysTot)));
    hout->SetBinContent(kMeanSysTot, mean);
    ...
    Fitter(hhigh_mult_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hhigh_mult_corr, hlo, hhi, integral, mean, extra);
    integral = TMath::Abs(integral - hout->GetBinContent(kYield));
    ...
    Fitter(hhard_mult_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hhard_mult_corr, hlo, hhi, integral, mean, extra);
    mean = TMath::Abs(mean - hout->GetBinContent(kMean));
    ...
    Fitter(hlow_mult_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hlow_mult_corr, hlo, hhi, integral, mean, extra);
    integral = TMath::Abs(integral - hout->GetBinContent(kYield));
    ...
    Fitter(hsoft_mult_corr, f, opt, range_min, range_max);
    ...
    YieldMean_IntegralMean(hsoft_mult_corr, hlo, hhi, integral, mean, extra);
    mean = TMath::Abs(mean - hout->GetBinContent(kMean));
    ...
    // pt cor + pt uncor
    logger.Get(TLogLevel::DEBUG) << "dN / dy = " << hout->GetBinContent(kYield) << " +- " << hout->GetBinContent(kYieldStat) << " +- " << hout->GetBinContent(kYieldSysTot) << " ( + " << hout->GetBinContent(kYieldSysHiCorr) << " - " << hout->GetBinContent(kYieldSysLoCorr) << ") " << "\n<pT> = " << hout->GetBinContent(kMean) << " +- " << hout->GetBinContent(kMeanStat) << " + " << hout->GetBinContent(kMeanSysTot) << " ( + " << hout->GetBinContent(kMeanSysHardCorr) << " - " << hout->GetBinContent(kMeanSysSoftCorr) << ") ";
  ...
  YieldMean_LowExtrapolationHisto(TH1 *h, TF1 *f, double min, double binwidth)
  {
    ...
    if (binlo == -1)
      ::Fatal("YieldMean_LowExtrapolationHisto", "Lower bin is undefined !?");
  ...
  YieldMean_HighExtrapolationHisto(TH1 *h, TF1 *f, double max, double binwidth)
  {
    ...
    if (binhi == -1)
      ::Fatal("YieldMean_HighExtrapolationHisto", "Higher bin is undefined !?");
    if (max < hi)
    {
  ...
  YieldMean_ReturnExtremeHisto(TH1 *hin, const char *hout_title, float sign)
  {
    ...
    if (ptlow < 0. || pthigh < 0.)
      ::Fatal("YieldMean_ReturnExtremeHisto", "Problem in the determination of the pt region");
  ...
  void YieldMean_IntegralMean(TH1 *hdata, TH1 *hlo, TH1 *hhi, double &integral, double &mean, double &extra, Bool_t printinfo)
  {
    ...
    extra = E;
    if (printinfo)
      logger.Get(TLogLevel::DEBUG) << "data only = " << dataonly << " total = " << I << " ratio= " << dataonly / I;
  }
  ...
  int Fitter(TH1 *histo, TF1 *func, Option_t *opt, float range_min, float range_max)
  {
    int fitres;
    int trials = 0;
    do
    {
      fitres = histo->Fit(func, opt, "", range_min, range_max);
