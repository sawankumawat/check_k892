            ...
            */
            ...
            for (int imult = 0; imult < 1; imult++)
            {
                ...
                // for (Int_t ip = 0; ip < 4; ip++) // start pt bin loop
                {
                    ...
                    hfsig->GetXaxis()->SetRangeUser(1.00, 2.20);
                    hfsig->Draw("e");
                    TLine *linesig = new TLine(1.0, 0, 2.50, 0);
                    linesig->SetLineColor(kRed);
                    linesig->SetLineStyle(2);
                    ...
                    TLegend *lp2 = DrawLegend(0.55, 0.73, 0.85, 0.92);
                    ...
                    lp2->AddEntry((TObject *)0, "pp, #sqrt{#it{s}} = 13.6 TeV", "");
                    lp2->AddEntry((TObject *)0, "FT0M, 0-100%, |#it{y}| < 0.5", "");
                    // lp2->AddEntry((TObject *)0, "|#it{y}| < 0.5", "");
                    lp2->AddEntry((TObject *)0, Form("%.1f < #it{p}_{T} < %.1f GeV/#it{c}", lowpt, highpt), "");
                    if (ip == 0)
                        lp2->Draw("same");

                    t2->DrawLatex(0.58, 0.69, Form("#bf{%.1f < #it{p}_{T} < %.1f GeV/c}", lowpt, highpt));
                    ...
                    fHistTotal[ip]->GetYaxis()->SetTitleOffset(1.5);
                    fHistTotal[ip]->GetYaxis()->SetTitle(Form("Counts / (%.0f MeV/#it{c}^{2})", binwidth_file * 1000));
                    fHistTotal[ip]->SetMaximum(1.3 * fHistTotal[ip]->GetMaximum());
                    fHistTotal[ip]->GetXaxis()->SetTitle("#it{M}_{K^{0}_{s}K^{0}_{s}} (GeV/#it{c}^{2})");
                    fHistTotal[ip]->GetXaxis()->SetRangeUser(1.00, 2.20);
                    fHistTotal[ip]->Draw("E");
                    ...
                    TLegend *leg = new TLegend(0.53, 0.62, 0.92, 0.72);
                    leg->SetFillStyle(0);
                    leg->SetBorderSize(0);
                    leg->SetTextFont(42);
                    leg->SetTextSize(0.03);
                    // leg->SetTextSize(0.055);
                    leg->AddEntry(fHistTotal[ip], "Same-event K^{0}_{s} K^{0}_{s} pairs", "p");
                    string bkgname = (kResBkg == "MIX") ? "Mixed event" : "Rotational background";
                    leg->AddEntry(hfbkg, bkgname.c_str(), "p");
                    // if (kResBkg == "MIX")
                    hbkg_nopeak->SetLineWidth(0);
                    // leg->AddEntry(hbkg_nopeak, "Normalization region", "f");
                    ...
                    // t2->DrawLatex(0.6, 0.71, Form("#bf{%.1f < #it{p}_{T} < %.1f GeV/c}", lowpt, highpt));
