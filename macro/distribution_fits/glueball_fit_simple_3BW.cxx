...
Double_t BWsumMassDepWidth3BW(double *x, double *par);
Double_t BWsumMassDepWidth4BW_exponential(double *x, double *par);

void glueball_fit_simple_3BW()
{
    ...
    for (int isysvars = 0; isysvars < 1; isysvars++)
    {
        ...
        // const string kvariation1 = "_fitHigh2p23";
        // const string kvariation1 = "_normLeft"; //2.4-2.5
        // const string kvariation1 = "_normRight"; // 2.6-2.7 (fit in 1.09-2.15 range)
        //======== For fit function variation =============================
        // const string kvariation1 = "_FitChKstar";
        // const string kvariation1 = "_FitExpoHERA";
        const string kvariation1 = "_Fit3rBW";
        ...
        string resonance_mass[] = {"1270", "1525", "1710"};
...
// //SetCanvasStyle(c_reducedFit, 0.14, 0.03, 0.05, 0.13);
                    ...
                    BEexpol_initial->FixParameter(7, f1710Mass);
                    BEexpol_initial->FixParameter(8, f1710Width);
                    ...
                    //=============================================================//
                    for (int iparams = 0; iparams < 13; iparams++)
                    {
                        BEexpol->SetParameter(iparams, BEexpol_initial->GetParameter(iparams));
                    }

                    vector<vector<double>> par_limits = {{1, 1 * f1270Width}, {2, 3 * f1270WidthErr}, {4, 1 * f1525Width}, {5, 3 * f1525WidthErr}, {7, 0.45 * f1710Width}, {8, 3 * f1710WidthErr}};
                    ...
                    TF1 *onlyBW = new TF1("onlyBW", BWsumMassDepWidth3BW, BEexpol->GetXmin(), BEexpol->GetXmax(), 9);
                    TF1 *onlyBW_clone = new TF1("onlyBW_clone", BWsumMassDepWidth3BW, BEexpol->GetXmin(), BEexpol->GetXmax(), 9);
                    string parameter_names[] = {"f_{2}(1270) Amp", "f_{2}(1270) Mass", "f_{2}(1270) #Gamma", "f'_{2}(1525) Amp", "f'_{2}(1525) Mass", "f'_{2}(1525) #Gamma", "f_{0}(1710) Amp", "f_{0}(1710) Mass", "f_{0}(1710) #Gamma"};
                    for (int i = 0; i < 9; i++)
                    {
                    ...
                    // onlyBW_clone->SetParLimits(3, 0.0, 1e6);   // norm1525
                    ...
                    // onlyBW_clone->FixParameter(7, f1710Mass);
                    onlyBW_clone->FixParameter(8, f1710Width);
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        singlefits[i] = (i < 2) ? new TF1(Form("singlef%d", i), single_BW_mass_dep_spin2, 1.00, 3.0, 3) : new TF1(Form("singlef%d", i), single_BW_mass_dep_spin0, 1.00, 3.0, 3);
...
// c->SaveAs((savepath + Form("/rBWfit_fit_%.2f_%.2f.png", fitlow, fithigh)).c_str());
                    ...
                    double resdual_bkg_par1 = BEexpol->GetParameter(9);
                    double resdual_bkg_par2 = BEexpol->GetParameter(10);
                    double resdual_bkg_par3 = BEexpol->GetParameter(11);
                    double resdual_bkg_par4 = BEexpol->GetParameter(12);

                    if (fitnorm1270 < 0 || fitnorm1525 < 0 || fitnorm1710 < 0)
                    {
...
// Now subtract the residual background and plot
                    ...
                    {
#ifdef b_massdepWidth_modifiedBoltzmann
                        singlefits1[i] = (i < 2) ? new TF1(Form("singlef%d", i), single_BW_mass_dep_spin2, 1.00, 2.5, 3) : new TF1(Form("singlef%d", i), single_BW_mass_dep_spin0, 1.00, 2.5, 3);
#else
                    ...
                    ltemp2->AddEntry(singlefits1[1], "f'_{2}(1525)", "l");
                    ltemp2->AddEntry(singlefits1[2], "f_{0}(1710)", "l");
                    TLatex lat2;
                    lat2.SetNDC();
#ifdef multiPanelPlots
...
// c2->SaveAs((savepath + "/rBWfit_residual_" + kvariation1 + ".png").c_str());
                    ...
                    TMatrixDSym cov1;
                    TMatrixDSym cov2;
                    cov.GetSub(0, 8, 0, 8, cov1);
                    cov.GetSub(9, 11, 9, 11, cov2);
                    ...
                    double yield1525Integral = singlefits[1]->Integral(fitRegion1525_low, fitRegion1525_high) / (ptBinWidth * binwidthfile * total_events);
                    double yield1710Integral = singlefits[2]->Integral(fitRegion1710_low, fitRegion1710_high) / (ptBinWidth * binwidthfile * total_events);

                    // Create individual covariance matrices for each resonance (3x3 each)
                    TMatrixDSym cov1270(3), cov1525(3), cov1710(3);
                    cov.GetSub(0, 2, 0, 2, cov1270); // f1270: parameters 0-2
                    cov.GetSub(3, 5, 3, 5, cov1525); // f1525: parameters 3-5
                    cov.GetSub(6, 8, 6, 8, cov1710); // f1710: parameters 6-8
                    ...
                    Double_t *para1525 = singlefits[1]->GetParameters();
                    Double_t *para1710 = singlefits[2]->GetParameters();
                    ...
                    double yield1525_err = singlefits[1]->IntegralError(fitRegion1525_low, fitRegion1525_high, para1525, cov1525_array) / (ptBinWidth * binwidthfile * total_events);
                    double yield1710_err = singlefits[2]->IntegralError(fitRegion1710_low, fitRegion1710_high, para1710, cov1710_array) / (ptBinWidth * binwidthfile * total_events);
...
Double_t BWsumMassDepWidth_exponential(double *x, double *par)
{
    return (BWsumMassDepWidth3BW(x, par) + exponential_bkg_3(x, &par[9]));
}
