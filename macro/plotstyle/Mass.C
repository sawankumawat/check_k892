...
/*
TGraphAsymmErrors * PlotGraph(Int_t NdataPoint=1, Int_t MarkerColor = 1, Int_t MarkerStyle = 20, Double_t *X, Double_t *ErXL, Double_t *ErXH, Double_t *Y, Double_t *ErYL, Double_t *ErYH)
{
...

{
  ...
  TFile *f0 = new TFile("f1Mix0_100_rebin1stbin.root","READ");  
  TH1D  *h0 = (TH1D*)f0->Get("hmass");

  for (int i=1;i<=h0->GetNbinsX();i++)
  {
  double relerror=h0->GetBinError(i)/h0->GetBinContent(i);
  cout<<i<<" "<<relerror<<endl;
  }
  
  h0->SetBinError(11,0.0012*h0->GetBinContent(11));
  h0->SetBinError(13,0.001*h0->GetBinContent(13));
  
  
  TH1D *h_0=Plothisto(h0, kRed, 24); //v2default


  TF1 *pdg = new TF1("frac","pol0",0.0,15.0);
  pdg->SetFillColor(19);
  pdg->SetFillStyle(0);
  pdg->SetMarkerStyle(20);
  pdg->SetMarkerSize(1.5);
  pdg->SetLineColor(kAzure+2);
  pdg->SetLineStyle(9);
  pdg->SetLineWidth(3);
  pdg->SetChisquare(2.619102);
  pdg->SetNDF(4);
  pdg->SetParameter(0,1.281);
  pdg->SetParError(0,0.0005);

 
 TH1D *h_dummy=new TH1D("h_dummy","h_dummy",1,0.0,15.0);
 h_dummy=Plothisto(h_dummy,1,1);
 h_dummy->SetMinimum(1.26);
 h_dummy->SetMaximum(1.30);
...
TLegend *lp2=DrawLegend(0.6,0.65,0.8,0.82);
...
lp2->AddEntry((TObject*)0,"0-100 %","");
 lp2->AddEntry(pdg,"PDG","l");
  ...
  TFile *file = new TFile("Mass.root","RECREATE");
  file->cd();
  h0->Write();
  file->Close();
