...
void compare_yield_inelgt0_minBias()
{
    ...
    TString outputPath = path2 + "/minBias_compare";
    ...
    for (int imult = 0; imult < numofmultbins; imult++)
    {
        ...
        gRun2_ratio[imult] = (TGraphErrors *)fpub->Get(Form("Table %d/Graph1D_y1", imult + 1 + numofmultbins)); // ratio of given multiplicity class to 0-100% class
    ...
    for (int i = 0; i < gRun2_minBias[7]->GetN(); i++) // took 4th for less error bar
    {
        double x_run2, yield_run2, x_error, y_error_run2;
        gRun2_minBias[7]->GetPoint(i, x_run2, yield_run2);
    ...
    TLegend *leg = new TLegend(0.4, 0.68, 0.9, 0.91);
    SetLegendStyle(leg);
    ...
    gratio1->GetHistogram()->SetMaximum(gratio1->GetHistogram()->GetMaximum() * 1.2);
