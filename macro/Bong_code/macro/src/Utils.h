...
class TTList : public TObject {
 public:
  TObject* Get(std::string name) {
    TObject* obj{nullptr};
    if (this->InheritsFrom(TList::Class()))
      obj = dynamic_cast<TList*>((TObject*)this)->FindObject(name.data());
    else if (this->InheritsFrom(TDirectoryFile::Class()))
      obj = dynamic_cast<TDirectoryFile*>((TObject*)this)->Get(name.data());
    else
      std::cout << "Not a list nor a directory, returning nullptr."
                << std::endl;

    utils::Requires(obj, name);
    return obj;
  }
};
...
constexpr bool ValueInInterval(const double val, const double min,
                               const double max) {
...
double QuadratureSum(std::vector<double>& args) {
  double sum = 0.;
  for (double& val : args) sum += Sq(val);
  return std::sqrt(sum);
}
...
double Ztest(const double mu0, const double sig0, const double mu1,
             const double sig1, const double corr = 0.) {
...
void Divide(TH1* h, TGraph* gr) {
  for (int i = 1; i <= h->GetNbinsX(); ++i) {
    if (h->GetBinContent(i) == 0 ||
        std::abs(gr->Eval(h->GetBinCenter(i))) < FLT_MIN * 10.f) {
      continue;
    }
