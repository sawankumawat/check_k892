...
void plot_spectra(vector<TString> paths, vector<string> legendnames = {}, bool isSinglePanel = false, bool isINEL = false, TString lheader = "");
...
void compare_ClosureRatio()
{
    ...
    // vector<string> QAVariation = {"_MC_closure", "_MC_closure_MID0p3"}; //(Train no. 673285)
    // vector<string> legendnames = {"Default", "MID 0.3#sigma"};

    vector<string> QAVariation = {"_MC_closure", "_MC_closure_MID0p3", "_MC_closure_MID", "_MC_closure_WithoutTOFShift", "_MC_closure_NoITSROF"}; //(Train no. 674418: All with mother global id check)
    vector<string> legendnames = {"Default", "MID 0.3#sigma", "MID 1#sigma", "Without TOF Shift", "No ITS ROF"};
    TString lheader = "Mother GlobalID check";

    std::vector<TString> paths;
    for (const auto &variation : QAVariation)
    {
        paths.emplace_back(Form("/home/sawan/check_k892/output/kstar/LHC22o_pass7/MC_closure/674418/kstarqa%s/hInvMass/MC_closure_plots/", variation.c_str()));
    }
    ...
    plot_spectra(paths, legendnames, isSinglePanel, isINEL, lheader);
}
...
void plot_spectra(vector<TString> paths, vector<string> legendnames = {}, bool isSinglePanel = false, bool isINEL = false, TString lheader = "")
{
    ...
    {
        fspectra[ifiles] = new TFile((paths[ifiles] + "MCClosure.root"), "read");
        ...
        for (int imult = 0; imult < numofmultbins + 1; imult++)
        {
            ...
            hmult[imult].push_back((TH1F *)fspectra[ifiles]->Get(Form("mult_%.0f-%.0f/Ratio", multlow, multhigh)));
    ...
    for (int imult = 0; imult < numofmultbins + 1; imult++)
    {
        ...
        for (int ifiles = 0; ifiles < totalfiles; ifiles++)
        {
            ...
            hmult[imult][ifiles]->GetYaxis()->SetTitle("(Reconstructed / True) Yield");

            hmult[imult][ifiles]->GetYaxis()->SetTitleOffset(1.59);
            hmult[imult][ifiles]->GetXaxis()->SetTitleOffset(1.4);
            hmult[imult][ifiles]->GetYaxis()->SetRangeUser(0.5, 1.5);
        ...
        c1->SaveAs(Form("%s/MCClosure_compare_%.0f-%.0f.png", paths[0].Data(), multlow, multhigh));
    }
}
