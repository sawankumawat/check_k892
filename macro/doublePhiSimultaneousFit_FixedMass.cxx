...
struct GlobalChi2
{
    ...
    // Global parameters mapping:
    // [0]: Shared Width
    // [1..5]: Yield_1, p0_1, p1_1, p2_1, p3_1
    // [6..10]: Yield_2, p0_2, p1_2, p2_2, p3_2
    // [11..15]: Yield_3, p0_3, p1_3, p2_3, p3_3
    double operator()(const double *par) const
    {
        double p1[7] = {par[1], FIXED_MASS, par[0], par[2], par[3], par[4], par[5]};
        double p2[7] = {par[6], FIXED_MASS, par[0], par[7], par[8], par[9], par[10]};
        double p3[7] = {par[11], FIXED_MASS, par[0], par[12], par[13], par[14], par[15]};
...
void doublePhiSimultaneousFit_FixedMass()
{
    gStyle->SetOptFit(0);
    gStyle->SetOptStat(0);
    ...
    // -----------------------------------------------------------------
    // Stage 3: Simultaneous Fit Across All Periods
    // Shared Width, Fixed Mass = 2.6902, Independent Yields & Backgrounds
    // -----------------------------------------------------------------
    cout << "\n=======================================================" << endl;
    ...
    fitter.Config().SetParamsSettings(16, std::vector<double>(16, 0.0).data());

    // Parameter [0]: Shared floating Width across all periods
    fitter.Config().ParSettings(0).SetName("SharedWidth");
    fitter.Config().ParSettings(0).SetValue(0.03);
    fitter.Config().ParSettings(0).SetLimits(0.005, 0.15);

    // Seed Stage 3 parameters using Stage 1 & Stage 2 results
    for (int i = 0; i < 3; ++i)
    {
        int baseIndex = 1 + i * 5;
    ...
    fitter.FitFCN(16, globalChi2, 0, totalNPoints, true);
    ROOT::Fit::FitResult result = fitter.Result();

    double sharedWidth = result.Value(0);
    double sharedWidthErr = result.Error(0);
    ...
    for (int i = 0; i < nPeriods; ++i)
    {
        cMulti->cd(i + 1);
        int baseIndex = 1 + i * 5;
        double simulYield = result.Value(baseIndex);
        double simulYieldErr = result.Error(baseIndex);

        fitSimulVec[i]->SetParameters(simulYield, FIXED_MASS, sharedWidth,
        ...
        latex.DrawLatex(0.20, 0.85, Form("Period: LHC%s", periods[i].c_str()));
        latex.DrawLatex(0.20, 0.80, Form("Fixed Mass = %.4f GeV/c^{2}", FIXED_MASS));
        latex.DrawLatex(0.20, 0.75, Form("Shared #Gamma = %.4f #pm %.4f GeV/c^{2}", sharedWidth, sharedWidthErr));
        latex.DrawLatex(0.20, 0.70, Form("Yield = %.1f #pm %.1f", simulYield, simulYieldErr));
    }
