...
int ReweightEfficiency(TH1D *M, TF1 *F, TH1D *G, TH1D *R, TFile *file, int test = 0, int save = 2, double tolerance = 0.001)
{
  ...
  */
  ...
  if (!M)
  {
    cerr << "Error in ReweightEfficiency(): missing input: measured pT spectrum" << endl;
    return 1;
  }
  if (!F)
  {
    cerr << "Error in ReweightEfficiency(): missing input: fit of measured pT spectrum" << endl;
    return 1;
  }
  if (!G)
  {
    cerr << "Error in ReweightEfficiency(): missing input: generated pT spectrum from simulation" << endl;
    return 1;
  }
  if (!R)
  {
    cerr << "Error in ReweightEfficiency(): missing input: reconstructed pT spectrum from simulation" << endl;
    return 1;
  }

  char mname[500];
  sprintf(mname, "%s", M->GetName());
  char fname[500];
  sprintf(fname, "%s", F->GetName());
  char gname[500];
  sprintf(gname, "%s", G->GetName());
  char rname[500];
  sprintf(rname, "%s", R->GetName());

  cout << "using ReweightEfficiency():\n  M=" << mname << "\n  F=" << fname << "\n  G=" << gname << "\n  R=" << rname << endl;

  if (save && !file)
  {
    cerr << "Warning in ReweightEfficiency(): flag save=" << save << " but no output file given" << endl;
    save = 0;
  }
  if (test)
    cerr << "Warning in ReweightEfficiency(): running in test mode (not using fit options M or E)" << endl;
  ...
  { // iterate
    ...
    if (diff < tolerance)
    {
      cerr << "re-weighting finished successfully" << endl;
      break;
    }
    if (i == imax - 1)
    {
      cerr << "Error in ReweightEfficiency(): maximum number of iterations (" << imax << ") reached and results have not stabilized, diff=" << diff << endl;
      status = 2;
      break;
    }
...
void isSameBinning(TH1D *h0, TH1D *h1)
{
  ...
  for (j = 1; j <= h0->GetNbinsX(); j++)
  {
    ...
    if (fabs(A0 - A1) < 1.e-6 && fabs(B0 - B1) < 1.e-6)
      continue;
    else
    {
      cerr << "Error in ReweightEfficiency(): mismatched bins detected for histograms " << h0->GetName() << " and " << h1->GetName() << endl;
