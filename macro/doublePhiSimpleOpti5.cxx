...
void doublePhiSimpleOpti5()
{
    ...
    for (int intervalBin = 0; intervalBin < totalIntervals; ++intervalBin)
    {
        ...
        fitBW->Draw("same");
        cInvMass->SaveAs(savepath + Form("/DoublePhiFit_deltaM_%d.png", intervalBin));
    ...
    //     fitFcn->SetParameter(4, 3.6e3);      // Pol2 p0
    //     fitFcn->SetParLimits(4, 1e2, 3e5);   // Pol2 p0
    //     fitFcn->SetParameter(5, 1.1e5);      // Pol2 p1
    //     fitFcn->SetParLimits(5, 1e2, 3e7);   // Pol2 p1
    //     fitFcn->SetParameter(6, -1.0e6);     // Pol2 p2
    //     fitFcn->SetParLimits(6, -1e9, -1e3); // Pol2 p2
    ...
    // latex->DrawLatex(0.25, 0.83, "Fit function: Voigtian + Pol2");
    // latex->DrawLatex(0.25, 0.75, "Purity window: #it{M}_{#Phi} #pm 0.005 GeV/#it{c}^{2}");
