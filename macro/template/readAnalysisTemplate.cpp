...
void readAnalysisTemplate()
{
    ...
    {
        cinv[ip] = new TCanvas(Form("cinv%d", ip), TString::Format("cinv_pt_%2.1f-%2.1f", pT_bins[ip], pT_bins[ip + 1]).Data(), 10, 10, 1440, 720);
        SetCanvasStyle(cinv[ip], 0.15, 0.05, 0.08, 0.13);

        cSigbkg[ip] = new TCanvas(Form("cSigbkg%d", ip), TString::Format("cSigbkg_pt_%2.1f-%2.1f", pT_bins[ip], pT_bins[ip + 1]).Data(), 1440, 720);
    ...
    // Open input files
    // =========================================================================
    TFile *fInputFile = new TFile("725720.root", "READ");
    ...
    for (Int_t ip = pt_start; ip < pt_end; ip++)
    {
        ...
        // =====================================================================
        int lbincent = fHistUnlike->GetAxis(0)->FindBin(lowcent + 1e-5);
        int hbincent = fHistUnlike->GetAxis(0)->FindBin(highcent - 1e-5);
        ...
        if (kResBkg == "MIX")
        {
            lbincent = fHistMix->GetAxis(0)->FindBin(lowcent + 1e-5);
            hbincent = fHistMix->GetAxis(0)->FindBin(highcent - 1e-5);
        ...
        {
            lbincent = fHistLikeMM->GetAxis(0)->FindBin(lowcent + 1e-5);
            hbincent = fHistLikeMM->GetAxis(0)->FindBin(highcent - 1e-5);
            ...
            fHistbkgMM[ip]->SetName(Form("fHistLikeMM_%d", ip));

            lbincent = fHistLikePP->GetAxis(0)->FindBin(lowcent + 1e-5);
            hbincent = fHistLikePP->GetAxis(0)->FindBin(highcent - 1e-5);
            ...
            fHistbkgPP[ip]->SetName(Form("fHistLikePP_%d", ip));
        ...
        {
            lbincent = fHistRotated->GetAxis(0)->FindBin(lowcent + 1e-5);
            hbincent = fHistRotated->GetAxis(0)->FindBin(highcent - 1e-5);
            ...
            fHistRotated1D[ip]->SetName(Form("fHistRotated_%d", ip));
