...
void doublePhiTemplateBWExpol3()
{
    ...
    for (int ibin = 0; ibin < totalBins; ibin++)
    {
        ...
        // if (!has_valid_seed)
        {
            ...
            f2D->FixParameter(3, 1.01983); // Mass peak
            // f2D->SetParLimits(3, 1.016, 1.025);
            f2D->FixParameter(4, 0.007077); // Width
        ...
        h2D_Mass->Fit(f2D, "QERN");
        TFitResultPtr fitResult = h2D_Mass->Fit(f2D, "QERSN");
        // TFitResultPtr fitResult = h2D_Mass->Fit(f2D, "REBMS");
        ...
        //          { return pow(x[0], p5) * TMath::Exp(p6 * x[0] + p7 * x[0] * x[0] + p8 * x[0] * x[0] * x[0]); }, 1.0, 1.04, 0);
        ...
        // ================================================
        // =====1D PROJECTIONS OF PHI INVARIANT MASS=======
        // ================================================
        TH1D *h1D_m1 = h2D_Mass->ProjectionY(Form("h1D_m1_bin%d", ibin));
        TH1D *h1D_m2 = h2D_Mass->ProjectionX(Form("h1D_m2_bin%d", ibin));

        TCanvas *cTemph1D = new TCanvas(Form("cTemph1D_bin%d", ibin), "1D Mass Projections", 720, 720);
        SetCanvasStyle(cTemph1D, 0.18, 0.05, 0.07, 0.15);
        SetHistoQA(h1D_m1);
        h1D_m1->GetXaxis()->SetTitle("#it{M}_{#phi2} (GeV/#it{c}^{2})");
        h1D_m1->GetYaxis()->SetTitle("Counts / bin");
        h1D_m1->GetXaxis()->SetNdivisions(505);
        h1D_m1->GetXaxis()->SetRangeUser(1.0, 1.04);
        h1D_m1->Draw("E");
        cTemph1D->SaveAs(savepath + Form("/1DFits/1D_Mass_m1_bin%d.png", ibin));

        TCanvas *cTemph1D_m2 = new TCanvas(Form("cTemph1D_m2_bin%d", ibin), "1D Mass Projections", 720, 720);
        SetCanvasStyle(cTemph1D_m2, 0.18, 0.05, 0.07, 0.15);
        SetHistoQA(h1D_m2);
        h1D_m2->GetXaxis()->SetTitle("#it{M}_{#phi1} (GeV/#it{c}^{2})");
        h1D_m2->GetYaxis()->SetTitle("Counts / bin");
        h1D_m2->GetXaxis()->SetNdivisions(505);
        h1D_m2->GetXaxis()->SetRangeUser(1.0, 1.04);
        h1D_m2->Draw("E");
        cTemph1D_m2->SaveAs(savepath + Form("/1DFits/1D_Mass_m2_bin%d.png", ibin));
        ...
        //         double bkg = pow(m, p5) * TMath::Exp(p6 * m + p7 * m * m + p8 * m * m * m);
