...
namespace
{
  ...
  {
    return PhiPuritySignalModel(x, p) + PhiPurityBackgroundModel(x, p + 3);
  }
                             ...
                             double &background)
  {
    signal = 0.0;
    background = 0.0;
    if (!hMass || !parameters || !(windowMax > windowMin))
      return;
      ...
      const char *outPrefix)
  {
    ...
    if (!hMassVsPt || !(purityHalfWidth > 0.0) || !outPrefix ||
        ...
        !(requestedFitMax > requestedFitMin))
      return out;
    ...
    for (int ipt = 0; ipt < nPtBins; ++ipt)
    {
      ...
      for (int ip = 0; ip < 7; ++ip)
        parameters[ip] = total->GetParameter(ip);
  ...
  {
    if (!hSparse || hSparse->GetNdimensions() <= kAxisRapidity)
      return false;
                            ...
                            const char *outPrefix)
  {
    ...
    if (coeffStore1.size() > 0)
      res.c1m1 = coeffStore1[0]->getVal();
    if (coeffStore1.size() > 1)
      res.c2m1 = coeffStore1[1]->getVal();
    if (coeffStore1.size() > 2)
      res.c3m1 = coeffStore1[2]->getVal();
    if (coeffStore2.size() > 0)
      res.c1m2 = coeffStore2[0]->getVal();
    if (coeffStore2.size() > 1)
      res.c2m2 = coeffStore2[1]->getVal();
    if (coeffStore2.size() > 2)
      res.c3m2 = coeffStore2[2]->getVal();
                                ...
                                const Fit2DResult &fit)
  {
    ...
    const double total = aSS + std::max(0.0, fit.nSB) * s1 * b2 + std::max(0.0, fit.nBS) * b1 * s2 + std::max(0.0, fit.nBB) * b1 * b2;
                                   ...
                                   const char *savePrefix = "")
  {
    ...
    if (fixShape)
    {
      c1m1.setConstant(true);
      c2m1.setConstant(true);
      c3m1.setConstant(true);
      c1m2.setConstant(true);
      c2m2.setConstant(true);
      c3m2.setConstant(true);
    }
    ...
    res.nBSErr = nBS.getError();
    res.nBBErr = nBB.getError();
    res.c1m1 = c1m1.getVal();
    res.c2m1 = c2m1.getVal();
    res.c3m1 = c3m1.getVal();
    res.c1m2 = c1m2.getVal();
    res.c2m2 = c2m2.getVal();
    res.c3m2 = c3m2.getVal();
      ...
      double mPairHi)
  {
    ...
    if (!axMPair || !axPt || !axY || !axM1 || !axM2 || !axDM)
      return out;
    ...
    for (Long64_t ib = 0; ib < hSparse->GetNbins(); ++ib)
    {
      ...
      if (mPair < mPairLo || mPair >= mPairHi)
        continue;
      if (pt < ptMin || pt >= ptMax)
        continue;
      if (rapidity < gRapidityMin || rapidity >= gRapidityMax)
        continue;
      if (m1 < phiMassMin || m1 >= phiMassMax)
        continue;
      if (m2 < phiMassMin || m2 >= phiMassMax)
        continue;
    ...
    out.fitVarDiag = dSel_dNSS * dSel_dNSS * eNSS * eNSS + dSel_dNSB * dSel_dNSB * eNSB * eNSB + dSel_dNBS * dSel_dNBS * eNBS * eNBS + dSel_dNBB * dSel_dNBB * eNBB * eNBB;
      ...
      const char *saveFitDir = "")
  {
    ...
    for (int ib = 1; ib <= nPairBins; ++ib)
    {
      ...
      std::cout << Form(
                       ...
                       static_cast<long long>(sel.usedSparseBins))
                << std::endl;
    }
...
namespace
{
  ...
  double VoigtRawIntegral(double xmin,
                          ...
                          double resolution)
  {
    if (!(xmax > xmin) || !(gamma > 0.0) || !(resolution > 0.0))
      return 0.0;
    ...
    double sum = VoigtRaw(xmin, mean, gamma, resolution) + VoigtRaw(xmax, mean, gamma, resolution);
                                  ...
                                  int component)
  {
    ...
    for (int i = 1; i <= h->GetNbinsX(); ++i)
    {
      ...
      const double ySig = nSig * VoigtRaw(x, sigMean, sigGamma, sigResolution) * dx;
  ...
  void DrawTemplateConstructionTwoPanel(TH1D *hTrueYield,
                                        ...
                                        const char *outPrefix)
  {
    if (!hTrueYield || !fTrueSmooth || !hNonSSYield || !fNonSSSmooth)
      return;
      ...
      double nominalOtherYield)
  {
    ...
    {
      const double x = hData->GetXaxis()->GetBinCenter(i);
      if (rejectHi > rejectLo && x > rejectLo && x < rejectHi)
        continue;
  ...
  TF1 *FitResidualVoigtian(TH1D *hResidual,
                           ...
                           double resolution)
  {
    if (!hResidual || residualFitHi <= residualFitLo)
      return nullptr;
    ...
    for (int i = 1; i <= hResidual->GetNbinsX(); ++i)
    {
      ...
      if (x <= residualFitLo || x >= residualFitHi)
        continue;
                                           ...
                                           int directBkgModel)
  {
    ...
    if (!hMass || hMass->Integral() <= 0.0 || !(fitMax > fitMin) ||
        !(gammaFixed > 0.0) || !(resolutionFixed > 0.0))
      return out;
                                              ...
                                              const char *outPrefix)
  {
    ...
    hSignal->SetMarkerSize(0.68);
    hSignal->SetLineColor(kRed + 1);
    hSignal->SetMarkerColor(kRed + 1);
    hOutsideScaled->SetLineColor(kBlue + 1);
    hOutsideScaled->SetLineWidth(3);
    hOutsideScaled->SetMarkerColor(kBlue + 1);
    hOutsideScaled->SetMarkerStyle(24);
    StyleAxis1D(hSignal);
      ...
      double rSHMFiducial)
  {
    ...
    if (r.nTruePhiPhiWindow > 0.0)
    {
      ...
      const double relX = r.nXWindow > 0.0
                              ? r.nXWindowErr / r.nXWindow
                              : 0.0;
      const double relPair = r.nTruePhiPhiWindowErr > 0.0
                                 ? r.nTruePhiPhiWindowErr / r.nTruePhiPhiWindow
                                 : 0.0;
                                              ...
                                              const char *outPrefix)
  {
    ...
    legP0->SetBorderSize(0);
    legP0->SetFillStyle(0);
    legP0->SetTextSize(0.030);
      ...
      double xResolution = 0.015)
  {
         ...
         TString(phiMassVsPtName).IsNull() || !(phiFitMax > phiFitMin)))
    {
      std::cerr << "ERROR: phi-purity plotting requires a histogram name and "
                   "phiPurityHalfWidth>0 and phiFitMax>phiFitMin."
                << std::endl;
      return;
    }
    ...
    if (pairSignalMode == 1)
      pairSelectionMode = 0;
    else if (pairSignalMode == 2)
      pairSelectionMode = 1;
    else if (pairSignalMode == 3)
      pairSelectionMode = 3;
    ...
    if (!(normalFitDeltaM > 0.0))
      normalFitDeltaM = selectionCut;
    ...
    if (save2DFits)
      gSystem->mkdir(fitDir.Data(), kTRUE);
    ...
    if (selectionMode == 0)
      std::cout << "Control = stored DeltaM in [" << dmCtrlMin << "," << dmCtrlMax << "]." << std::endl;
    else
      std::cout << "Control = rectangular sideband/complement region." << std::endl;
    std::cout << "=====================================\n"
              << std::endl;
    ...
    if (!useTemplateFit)
    {
      ...
      if (hDirectSignal)
        hDirectSignal->Write();
      if (hDirectOutside)
        hDirectOutside->Write();
      if (h2DirectPhiPairWindow)
        h2DirectPhiPairWindow->Write();
