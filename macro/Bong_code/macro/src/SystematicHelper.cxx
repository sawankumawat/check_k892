   ...
   public:
    ...
    TH1* GetStdevRatio(bool AddMean = false) {
        ...
        for (int bin = 1; bin <= temp->GetNbinsX(); bin++) {
            ...
            for (auto const& hInput : hRatios){
                if(IsBarlowCheckPassed(hInput, hDefault, bin))
    ...
    void CalculateRatio() {
        ...
        for (auto const& hInput : hVariations) {
            ...
            for (int bin = 1; bin < hDivided->GetNbinsX()+1; bin++) {
                if(IsBarlowCheckPassed(temp, hDefault, bin))
                    hDivided->SetBinContent(bin, hDivided->GetBinContent(bin));
                else
    ...
    void CalculateDifference() {
        ...
        for (auto const& hInput : hVariations) {
            ...
            for (int bin = 1; bin < hSubtracted->GetNbinsX()+1; bin++) {
                if(IsBarlowCheckPassed(temp, hDefault, bin))
                    hSubtracted->SetBinContent(bin, hSubtracted->GetBinContent(bin));
                else
                    hSubtracted->SetBinContent(bin, 0);
            }
    ...
    bool IsBarlowCheckPassed(TH1* hInputb, TH1* hBaseb, int binb = 0){
        auto testResult = true;
        if (fSetBarlow) {
            double error =  // abs(Variation - Default) -> "Error"
                abs(hInputb->GetBinContent(binb) -
                    hBaseb->GetBinContent(binb));
            double errorFraction =  // abs(Variation - Default)/Deafult
                                    // - >"Error fraction"
                error / hInputb->GetBinContent(binb);
            double
                deltasigma =  // sqrt(abs(variation_stat.error^2 -
                                // default_stat.error^2)) -> "delta sigma"
                sqrt(abs(pow(hInputb->GetBinError(binb), 2) -
                            pow(hBaseb->GetBinError(binb), 2)));
            double deltasigmafraction =
                deltasigma / hInputb->GetBinContent(binb);
            if (fSetVerbose)
                cout
                    << "Variation value: " << hInputb->GetBinContent(binb)
                    << ", Default value: "
                    << hBaseb->GetBinContent(binb)
                    << ", Variation error: " << hInputb->GetBinError(binb)
                    << ", Default error: " << hBaseb->GetBinError(binb)
                    << endl;
    ...
    TCanvas* GetQAPlot(TString name = "RatioQA") {
        ...
        if (name.Contains("AbsDiffRatio")) {
            ...
            hAbsDiffRatios[0]->SetMaximum(fYmax);
        ...
        } else if (name.Contains("StdevRatio")){
            hRatios[0]->Draw("E");
            hRatios[0]->SetLineWidth(2);
            hRatios[0]->GetXaxis()->SetTitleSize(0.08);
            hRatios[0]->GetXaxis()->SetLabelSize(0.08);
