...
struct GlobalChi2
{
    ...
    // Global parameters mapping (17 total parameters):
    // [0]: Shared Mass
    // [1]: Shared Width
    // [2..6]   Period 1: Yield_1, p0_1, p1_1, p2_1, p3_1
    // [7..11]  Period 2: Yield_2, p0_2, p1_2, p2_2, p3_2
    // [12..16] Period 3: Yield_3, p0_3, p1_3, p2_3, p3_3
    double operator()(const double *par) const
    {
        // BWExpol signature: [Yield, Mass, Width, p0, p1, p2, p3]
        double p1[7] = {par[2], par[0], par[1], par[3], par[4], par[5], par[6]};
        double p2[7] = {par[7], par[0], par[1], par[8], par[9], par[10], par[11]};
        double p3[7] = {par[12], par[0], par[1], par[13], par[14], par[15], par[16]};
...
void doublePhiSimultaneousFit_SharedMassWidth()
{
    gStyle->SetOptFit(0);
    gStyle->SetOptStat(0);
    ...
    for (int i = 0; i < nPeriods; ++i)
    {
        ...
        // Fixed Mass = INIT_MASS, Fixed Bkg from Stage 1, Floating Width & Yield
        // -----------------------------------------------------------------
        ...
        fitIndivSB->FixParameter(1, INIT_MASS);
    ...
    // SHARED MASS & SHARED WIDTH, Independent Yields & Backgrounds
    // -----------------------------------------------------------------
    cout << "\n=======================================================" << endl;
    cout << " STAGE 3: SIMULTANEOUS GLOBAL FIT (SHARED MASS & WIDTH)" << endl;
    cout << "=======================================================" << endl;
    ...
    // 17 total parameters (2 shared + 3 * 5 per-period)
    fitter.Config().SetParamsSettings(17, std::vector<double>(17, 0.0).data());

    // Parameter [0]: Shared floating Mass across all periods
    fitter.Config().ParSettings(0).SetName("SharedMass");
    fitter.Config().ParSettings(0).SetValue(INIT_MASS);
    fitter.Config().ParSettings(0).SetLimits(2.65, 2.73);

    // Parameter [1]: Shared floating Width across all periods
    fitter.Config().ParSettings(1).SetName("SharedWidth");
    fitter.Config().ParSettings(1).SetValue(0.03);
    fitter.Config().ParSettings(1).SetLimits(0.005, 0.15);
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        for (int p = 1; p <= 4; ++p)
        {
            fitter.Config().ParSettings(baseIndex + p).SetName(Form("bkg_p%d_%s", p - 1, periods[i].c_str()));
            fitter.Config().ParSettings(baseIndex + p).SetValue(bkgPars[i][p - 1]);
        }
    }

    int totalNPoints = data1.Size() + data2.Size() + data3.Size();
    fitter.FitFCN(17, globalChi2, 0, totalNPoints, true);
    ROOT::Fit::FitResult result = fitter.Result();

    double sharedMass = result.Value(0);
    double sharedMassErr = result.Error(0);
    double sharedWidth = result.Value(1);
    double sharedWidthErr = result.Error(1);
    ...
    cout << Form("Shared Width     : %.4f +/- %.4f GeV/c2", sharedWidth, sharedWidthErr) << endl;

    for (int i = 0; i < nPeriods; ++i)
    {
        int baseIndex = 2 + i * 5;
        cout << Form("  -> Yield (LHC%s): %.1f +/- %.1f", periods[i].c_str(), result.Value(baseIndex), result.Error(baseIndex)) << endl;
    }
    cout << "=======================================================\n"
         << endl;
    ...
    for (int i = 0; i < nPeriods; ++i)
    {
        ...
        fitSimulVec[i]->SetParameters(simulYield, sharedMass, sharedWidth,
                                      result.Value(baseIndex + 1),
        ...
        fitBkgDraw->SetParameters(result.Value(baseIndex + 1), result.Value(baseIndex + 2),
                                  result.Value(baseIndex + 3), result.Value(baseIndex + 4));
        ...
        if (i == 0)
        {
            latex.DrawLatex(0.33, 0.82, Form("Shared M = %.4f #pm %.4f GeV/c^{2}", sharedMass, sharedMassErr));
            latex.DrawLatex(0.33, 0.77, Form("Shared #Gamma = %.4f #pm %.4f GeV/c^{2}", sharedWidth, sharedWidthErr));
